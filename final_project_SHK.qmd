...
collect_metrics(glm_fit_cv) #AUC=0.691
...
metrics(icu.4days_lr_pred_test, truth, .pred_class, `.pred_≧ 4days in ICU`) #AUC=0.688
```
...
rf_fit_cv<-rf_workflow|>
  fit_resamples(icu.4days_folds, control=control_resamples(save_pred=TRUE))
collect_metrics(rf_fit_cv) #AUC=0.723
...
metrics(icu.4days_rf_pred_test, truth, .pred_class, `.pred_≧ 4days in ICU`) #AUC=0.704 
```
...
bt_fit_cv <- bt_workflow |>
  fit_resamples(icu.4days_folds, control = control_resamples(save_pred = TRUE))
collect_metrics(bt_fit_cv) #AUC=0.694
...
metrics(icu.4days_xgb_pred_test, truth, .pred_class, `.pred_≧ 4days in ICU`) # AUC=0.680 
```

## Results {#sec-results}

Below are my main tables and figures that were presented above in the Methods section.
...
3.  **Machine learning I : Logistic regression**
...
4.  **Machine learning II : Random Forest**
...
5.  **Machine learning III : XGBoost**
...
The Random Forest model demonstrated the highest predictive accuracy for classifying whether an ALD patient's ICU stay would last 4 days or more, compared to logistic regression and XGBoost models. However, its performance was moderate, with an AUC of just over 70%. This highlights room for improvement and provides a basis for future optimization efforts.

The "top" significant variables that were used in each model were extracted (presented in the Methods section) for subsequent K-means clustering to classify ALD patients into subgroups based on ICU length of stay (≧ 4days, \<4days) and ICU deaths. Each model identified different sets of top variables, some of which overlap. I would appreciate your insight on whether I should select the top variables from a specific model or include all top variables from all three models moving forward. Your feedback and suggestions are greatly appreciated! Thank you :)
